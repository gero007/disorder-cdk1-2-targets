...
violin_simpleDisoPerc<-ggplot(Holt_data,aes(y = predDisoPerc,x=target_all)) + geom_violin(aes(fill= target_all),alpha=.5,bw=5) + geom_boxplot(color="darkslategray",width=0.2) +
...
violin_liteDisoPerc<-ggplot(Holt_data,aes(y = predLiteDisoPerc,x=target_all)) + geom_violin(aes(fill= target_all),alpha=.5,bw=5) + geom_boxplot(color="darkslategray",width=0.2) +
theme_linedraw() + theme(plot.title = element_text(hjust = 0.5),legend.position = "none",text = element_text(size=20)) +
scale_y_continuous(name="% of predicted Disorder",limits = c(0,105)) +
...
violin_spotDisoPerc<-ggplot(Holt_data,aes(y = predSpotDisoPerc,x=target_all)) + geom_violin(aes(fill= target_all),alpha=.5,bw=5) + geom_boxplot(color="darkslategray",width=0.2) +
theme_linedraw() + theme(plot.title = element_text(hjust = 0.5),legend.position = "none",text = element_text(size=20)) +
scale_y_continuous(name="% of predicted Disorder",limits = c(0,105)) +
...
ggarrange(violin_simpleDisoPerc,violin_liteDisoPerc,violin_spotDisoPerc,nrow = 1, ncol = 3, align = "hv")
...
Holt_indexes_data$SpotDisoProb<-Holt_data$predSpotDisoPerc/100

Holt_consInDiso_Spot_ObsVsExp_table <- Holt_indexes_data[,c("SpotDisoProb","sequence","[S|T]P","SPOT","target_all")]
Holt_consInDiso_Spot_ObsVsExp_table$`nun[S|T]P` <- lapply(Holt_consInDiso_Spot_ObsVsExp_table$`[S|T]P`,function(x){if(x!=-1){return(length(x))}else{return(0)}})
Holt_consInDiso_Spot_ObsVsExp_table$Expected <- as.numeric(Holt_consInDiso_Spot_ObsVsExp_table$`nun[S|T]P`)*as.numeric(Holt_consInDiso_Spot_ObsVsExp_table$SpotDisoProb)

# if expected is 0 there is no consensus or there is no Disorder regions. In any case those data points are not important because observed will be 0 too
# removing those values
Holt_consInDiso_Spot_ObsVsExp_table<-subset(Holt_consInDiso_Spot_ObsVsExp_table,Expected!=0)

Obs_cons_diso <- numeric()
for (i in 1:nrow(Holt_consInDiso_Spot_ObsVsExp_table)) {

  indexDiso <- Holt_consInDiso_Spot_ObsVsExp_table$SPOT[[i]]
  indexCons <- Holt_consInDiso_Spot_ObsVsExp_table$`[S|T]P`[[i]]
  
  Obs_cons_diso[i] <- length(which(indexCons %in% indexDiso))
}
Holt_consInDiso_Spot_ObsVsExp_table$Observed <- Obs_cons_diso

ggplot(Holt_consInDiso_Spot_ObsVsExp_table,aes(x=Observed,y=Expected)) + geom_point(aes(color=target_all)) + geom_abline(slope = 1) + theme_linedraw() + theme(plot.title = element_text(hjust = 0.5),legend.position = "none",text = element_text(size=17)) +
        scale_y_continuous(name="Expected [S/T]P in disordered regions",limits = c(0,32)) + scale_x_continuous(name="Observed [S/T]P in disordered regions",limits = c(0,32)) + annotate(geom = "text",label="n = 1785",x=3,y=31)
