...
CDK1targetSacCer<-subset(CDK1targetSacCer[-294,],`P-Score`>=0)
...
```{r cdkTarget ,echo=FALSE,message=FALSE,warning=FALSE, fig.cap="Figure 6:" }

yeastDisoNONCDK1target<-yeastDiso[-match(subset(CDK1targetSacCer,`P-Score`>=2)$Entry,yeastDiso$acc),]
disoCDKtargetsData <- cbind(as.numeric(yeastDisoNONCDK1target$predDisoPercAll),rep("Non Cdk1 Targets",nrow(yeastDisoNONCDK1target)))
disoCDKtargetsData <- rbind(disoCDKtargetsData,cbind(yeastDisoCDK1target$predDisoPercCdkTargets,rep("Cdk1 Targets",nrow(yeastDisoCDK1target))))
disoCDKtargetsData <-as.data.frame(disoCDKtargetsData)
disoCDKtargetsData$percOfDisorderLiteRes <- c(yeastDisoNONCDK1target$predLiteDisoPercAll,yeastDisoCDK1target$predLiteDisoPercCdkTargets)
colnames(disoCDKtargetsData)<-c("percOfDisorderRes","group","percOfDisorderLiteRes")
disoCDKtargetsData$percOfDisorderRes <- as.numeric(as.character(disoCDKtargetsData$percOfDisorderRes))

ggplot(disoCDKtargetsData)+geom_density(aes(x = percOfDisorderRes,fill = group),alpha=.5)
