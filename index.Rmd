...
ggplot(data = stretchData,aes(x=method,y=stretchLength)) + geom_violin(aes(fill=method), alpha = 0.7,show.legend = FALSE,bw=10) +
    theme_minimal() +
...
UbxCdk1TargetsLiteDisorderedRegions <- getPredLiteDiso(df = yeastDiso_Ubersax_subset[which(Ubersax_data$`P-Score`>=2),],"indices")
UbxCdk1TargetsLiteStratContingencyArray <- getStratContingencyArray(df = yeastDiso_Ubersax_subset[which(Ubersax_data$`P-Score`>=2),],sequence_col = "sequence",disordered_index_list = UbxCdk1TargetsLiteDisorderedRegions)

woolf_test(UbxCdk1TargetsLiteStratContingencyArray)
ors <- apply(UbxCdk1TargetsLiteStratContingencyArray, 3,function(x){
     ft<-fisher.test(x)
     return(ft$estimate)
     })
...
fisherDF <- data.frame(Ubersax_data$Entry)
colnames(fisherDF) <- "acc"
fisherDF$target <- Ubersax_data$target
fisherDF$ORs <- apply(UbxAllLiteStratContingencyArray, 3,function(x){
     ft<-fisher.test(x)
     return(ft$estimate)
     })
fisherDF$pvalue <- apply(UbxAllLiteStratContingencyArray, 3,function(x){
     ft<-fisher.test(x)
     return(ft$p.value)
     })
fisherDF$logPvalue <- -log10(fisherDF$pvalue)
fisherDF$logOr <- log10(fisherDF$ORs)

fisherDF$logOr[is.infinite(fisherDF$logOr) & fisherDF$logOr > 0] <- 2
fisherDF$logOr[is.infinite(fisherDF$logOr) & fisherDF$logOr < 0] <- -2
...
ggplot(fisherDF, aes(x = logOr, y = logPvalue)) +
