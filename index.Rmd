...
```{r cpomposition ,echo=FALSE,message=FALSE,warning=FALSE}
yeastDiso_seq_Disorder <- character()

for (i in 1:nrow(yeastDiso)) {
  diso_seq <- character()
  for (row in 1:nrow(spotDisorderList[[yeastDiso$acc[i]]])) {
    startIdx <- as.numeric(spotDisorderList[[yeastDiso$acc[i]]][row,1])
    endIdx <- as.numeric(spotDisorderList[[yeastDiso$acc[i]]][row,2])
    if (spotDisorderList[[yeastDiso$acc[i]]][row,3]=="D") {diso_seq <- paste(diso_seq,substr(yeastDiso[i,"sequence"],startIdx,endIdx),sep = "")}
  }
  yeastDiso_seq_Disorder <- c(yeastDiso_seq_Disorder,diso_seq)
}
# names(yeastDiso_seq_Disorder) <- yeastDiso$acc  


yeastSeq<-Biostrings::AAStringSet(yeastDiso$sequence)
yeastComp<-colSums(alphabetFrequency(yeastSeq))[1:20]/sum(colSums(alphabetFrequency(yeastSeq))[1:20])

yeastSeqDiso <- Biostrings::AAStringSet(yeastDiso_seq_Disorder)
yeastCompDiso<-colSums(alphabetFrequency(yeastSeqDiso))[1:20]/sum(colSums(alphabetFrequency(yeastSeqDiso))[1:20])

differentialComp <- sort((yeastCompDiso-yeastComp)/yeastComp,decreasing = T)

ggplot()+geom_bar(aes(x=reorder(names(differentialComp),-differentialComp), y=differentialComp),stat="identity")+ylim(c(-0.75,0.75))
