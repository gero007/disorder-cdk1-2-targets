...
```{r scores ,echo=FALSE,message=FALSE,warning=FALSE, fig.cap="Figure 6:" }

scoresData <- CDK1targetSacCer[,c("Phosphorylation","P-Score","Protein Level","nmol","nEq_STP","nEq_STPXKR","target")]
...
ggplot(scoresData,aes(x=log10(`nmol`),y=log10(Phosphorylation)))+geom_point(aes(color=target))+ geom_smooth(method = "lm")

cor(subset(scoresData,`P-Score`>=2)$nmol,subset(scoresData,`P-Score`>=2)$Phosphorylation)
```
