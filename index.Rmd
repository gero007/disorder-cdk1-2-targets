...
yeastDiso$predSpotDisoPercAll <- getSpotPredDiso(yeastDiso,output = "percentage","acc")
...
ggplot(data = yeastDiso)+geom_density(aes(x=predLiteDisoPercAll,fill="MobiDB lite"),bins = 30,alpha=0.5,position = "identity")+geom_density(aes(x=predDisoPercAll,fill="Simple"),bins = 30,alpha=0.5,position = "identity") + theme_minimal() + xlab("% of disorder residues") + scale_fill_manual(name = "Predictor", values = c("Simple"="darkorange1","MobiDB lite" = "chartreuse3"))
```
...
```
In the logarithmic scale the difference of considering only the mass, or the number of nano moles/nano equivalents does not show striking effects on the distribution of the dots in the figure 6. Then, direct comparisons between the mass of protein meassured and the calculation of moles and equivalents were performed. Is possible to see some dispersion of the dots for high values of protein levels, but still a direct linear correlation between the grmas of protein meassured and the calculations for moles and equivalents. In logarithmic scale there is even a stronger dependency.
...
```{r  ,echo=FALSE,message=FALSE,warning=FALSE, fig.cap="Figure 8:",fig.width=12}
...
simpleDisoPerc<-ggplot(CDK1targetSacCer)+geom_density(aes(x = predDisoPercCdkTargets,fill = target),alpha=.5,bw=6)+ theme_linedraw()+theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(limits = c(0,0.05)) + scale_x_continuous(name="% of predicted Disorder",limits = c(0,100))+ggtitle("Simple Predictor")
liteDisoPerc<-ggplot(CDK1targetSacCer)+geom_density(aes(x = predLiteDisoPercCdkTargets,fill = target),alpha=.5,bw=4)+ theme_linedraw()+theme(plot.title = element_text(hjust = 0.5)) + theme(axis.title.y = element_blank())+ scale_y_continuous(limits = c(0,0.05)) + scale_x_continuous(name="% of predicted Disorder",limits = c(0,100)) + theme(axis.title.y = element_blank())+labs(title="MobiDB-Lite Predictor")
ggarrange(simpleDisoPerc,liteDisoPerc,nrow = 1, ncol = 2,common.legend = TRUE,legend = "bottom")
```
...
CDK1targetSacCer$predSpotDisoPerc <- yeastDiso$predSpotDisoPercAll[match(CDK1targetSacCer$Entry,yeastDiso$acc)]
```
...
Holt_MSdata <- read_delim("/home/gdubra/projects/cdk1_cyclinB1/phase_separation/disorder/Holt_CDK1targetSacCer.tsv",
"\t", escape_double = FALSE, trim_ws = TRUE)

#Add Uniprot accesion. 58 entries lost (out of 9067) due to ID conversion
Holt_MSdata <- Holt_MSdata[which(Holt_MSdata$ORF %in% uniprot_yeast$OLN),]

Holt_MSdata$Uniprot <- uniprot_yeast$`Swiss-Prot-AC`[base::match(Holt_MSdata$ORF,uniprot_yeast$OLN)]
