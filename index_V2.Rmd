...
# TEST -> MAPK added________
human_reshaped_disoPerc$target_mapk <- rep(human_data$target_mapk,length(human_disoPerc_cols_idx))
human_reshaped_disoPerc$target_mapk<- relevel(as.factor(human_reshaped_disoPerc$target_mapk),ref="Non mapk target")

human_reshaped_disoPerc$target_all <- rep(human_data$target_all,length(human_disoPerc_cols_idx))
human_reshaped_disoPerc$target_all<- relevel(as.factor(human_reshaped_disoPerc$target_all),ref="Phosphoproteins")
...
ggplot(human_reshaped_disoPerc_figure,aes(x=Predictor, y=`Percentage of disorder`)) +
geom_violin(aes(fill=target_mapk,color=target_mapk),position=position_dodge(0.9),bw=10,alpha=1,trim = F) +
geom_boxplot(aes(color=target_mapk),position=position_dodge(0.9),width=0.4,outlier.size = 0.0001) +
ggpubr::theme_classic2() +
theme(text = element_text(size=20),legend.position = "bottom",legend.title = element_blank(),axis.ticks.x = element_blank(),panel.grid.major.y = element_line(colour = "grey",linetype = "dashed")) +
scale_y_continuous(limits = c(-20, 135), breaks = c(seq(0, 100, by = 20))) +
scale_x_discrete(labels = c("IUPred", "SPOT","VSL2b")) + xlab(NULL) +
...
wilcox.test(`Percentage of disorder`~target_mapk,data=human_reshaped_disoPerc_figure,subset = Predictor=="iupl_perc",conf.int=TRUE)
wilcox.test(`Percentage of disorder`~target_mapk,data=human_reshaped_disoPerc_figure,subset = Predictor=="spot_perc",conf.int=TRUE)
wilcox.test(`Percentage of disorder`~target_mapk,data=human_reshaped_disoPerc_figure,subset = Predictor=="vsl_perc",conf.int=TRUE)


ggplot(human_reshaped_disoPerc_figure,aes(x=Predictor, y=`Percentage of disorder`)) +
geom_violin(aes(fill=target_all,color=target_all),position=position_dodge(0.9),bw=10,alpha=1,trim = F) +
geom_boxplot(aes(color=target_all),position=position_dodge(0.9),width=0.4,outlier.size = 0.0001) +
ggpubr::theme_classic2() +
theme(text = element_text(size=20),legend.position = "bottom",legend.title = element_blank(),axis.ticks.x = element_blank(),panel.grid.major.y = element_line(colour = "grey",linetype = "dashed")) +
scale_y_continuous(limits = c(-20, 135), breaks = c(seq(0, 100, by = 20))) +
scale_x_discrete(labels = c("IUPred", "SPOT","VSL2b")) + xlab(NULL) +
...
for (i in 1:length(human_idx_cols)) {
  humanStratContingencyArray <- getStratContingencyArray(df = subset(human_data,!is.na(MAPK_KINASE)),
                                                    sequence_col = "Sequence",
                                                    diso_index_col = human_idx_cols[i],
...
for (i in 1:length(human_idx_cols)) {
  humanStratContingencyArray <- getStratContingencyArray(df = subset(human_data,!is.na(AURK_KINASE)),
                                                    sequence_col = "Sequence",
                                                    diso_index_col = human_idx_cols[i],
...
for (i in 1:length(human_idx_cols)) {
  humanStratContingencyArray <- getStratContingencyArray(df = subset(human_data,!is.na(PLK_KINASE)),
                                                    sequence_col = "Sequence",
                                                    diso_index_col = human_idx_cols[i],
...
predictorsFisher_data<-as.data.frame(rbind(yeastIntersect_Fisher_data,human_Fisher_data,human_MAPK_Fisher_data,human_AURK_Fisher_data,human_PLK_Fisher_data,xenopus_Fisher_data))
...
# Test lollipop plots
predictorsCMH_data %>% unite(col = "group",organism,predictors,sep = "-",remove = F) %>% ggplot(aes(x=group, y=commonOR)) +
    geom_segment( aes(xend=group, yend=0)) +
    geom_point(aes(color=predictors),size=12) +
    theme_bw(base_size = 15) +
    coord_flip() +
    scale_color_manual(values = pal_jco()(10)[c(5,3,4)],guide=FALSE)+
    xlab("")
