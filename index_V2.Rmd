...
humanCDKtargets <- read_lines(file = "humanCDKtargets_mpi.txt")


# CHANGE!!!!!!!! if targets are share it only assings to the first one
...
xenopus_reshaped_disoPerc$cluster <- rep(xenopus_data$cluster,length(xenopus_disoPerc_cols_idx))
xenopus_reshaped_disoPerc$cluster<- relevel(as.factor(xenopus_reshaped_disoPerc$cluster),ref="Non Dynamic phosphorylation")
xenopus_reshaped_disoPerc <- xenopus_reshaped_disoPerc %>% mutate(dynamic=case_when(
    cluster != "Non Dynamic phosphorylation" ~ "Dynamic",
    TRUE ~ "Non dynamic"
))
xenopus_reshaped_disoPerc$dynamic<- relevel(as.factor(xenopus_reshaped_disoPerc$dynamic),ref="Dynamic")
...
geom_violin(aes(fill=cluster,color=cluster),position=position_dodge(0.9),bw=10,alpha=1,trim = F) +
geom_boxplot(aes(color=cluster),position=position_dodge(0.9),width=0.4,outlier.size = 0.0001) +
ggpubr::theme_classic2() +
theme(text = element_text(size=20),legend.position = "bottom",legend.title = element_blank(),axis.ticks.x = element_blank(),panel.grid.major.y = element_line(colour = "grey",linetype = "dashed")) +
scale_y_continuous(limits = c(-20, 135), breaks = c(seq(0, 100, by = 20))) +
scale_x_discrete(labels = c("IUPred", "SPOT","VSL2b")) + xlab(NULL) +
scale_fill_manual(values = pal_jco()(10)[1:5]) +
scale_color_manual(values = pal_jco()(10)[6:10],guide=FALSE)


ggplot(xenopus_reshaped_disoPerc_figure,aes(x=Predictor, y=`Percentage of disorder`)) +
geom_violin(aes(fill=dynamic,color=dynamic),position=position_dodge(0.9),bw=10,alpha=1,trim = F) +
geom_boxplot(aes(color=dynamic),position=position_dodge(0.9),width=0.4,outlier.size = 0.0001) +
ggpubr::theme_classic2() +
theme(text = element_text(size=20),legend.position = "bottom",legend.title = element_blank(),axis.ticks.x = element_blank(),panel.grid.major.y = element_line(colour = "grey",linetype = "dashed")) +
scale_y_continuous(limits = c(-20, 135), breaks = c(seq(0, 100, by = 20))) +
scale_x_discrete(labels = c("IUPred", "SPOT","VSL2b")) + xlab(NULL) +
